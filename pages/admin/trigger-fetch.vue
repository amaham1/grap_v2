...
type SourceName = 'festivals' | 'exhibitions' | 'welfare-services';
...
const results = ref<{
  [key in SourceName | 'welfareServices']?: { success: boolean; message: string }
}>({});

async function triggerFetch(sourceName: SourceName) {
  const key = sourceName === 'welfare-services' ? 'welfareServices' : sourceName;
