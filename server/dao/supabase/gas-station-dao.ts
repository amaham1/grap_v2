...
export async function batchUpsertGasStations(gasStations: GasStation[]) {
  ...
  return await batchUpsert('gas_stations', data, 50, 'opinet_id') // Cloudflare Workers 최적화를 위해 배치 크기 더 감소
}
...
export async function batchUpsertGasPrices(gasPrices: GasPrice[]) {
  ...
  return await batchUpsert('gas_prices', data, 50, 'opinet_id,price_date') // Cloudflare Workers 최적화를 위해 배치 크기 더 감소
}
